...
description: ""
access_date: 2026-08-30T01:55:50.104Z
current_date: 2026-08-30T01:55:50.104Z
---
...
## state\_mismatch - verification not found (database strategy)
...
## state\_mismatch - auth state cookie not found (cookie strategy)
...
## state\_mismatch - request expired
...
## state\_invalid - failed to decrypt or parse (cookie strategy)
...
## state\_security\_mismatch - state not persisted correctly (database strategy)
...
- **OAuth Proxy with mismatched secrets:** When using the [OAuth Proxy plugin](https://better-auth.com/docs/plugins/oauth-proxy) and production/preview have different `BETTER_AUTH_SECRET` values without a shared `secret` configured in the plugin options, the proxy's before hook fails to decrypt the state package, causing the regular callback handler to run and fail.
...
## state\_generation\_error - unable to create verification
...
| **Very high** | OAuth Proxy: different secrets on production vs preview | `state_security_mismatch` or `state_mismatch` | Configure a shared `secret` in the [OAuth Proxy plugin](https://better-auth.com/docs/plugins/oauth-proxy) |
