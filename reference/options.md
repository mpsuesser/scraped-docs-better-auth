...
title: "Options"
description: ""
access_date: 2026-09-12T23:24:04.130Z
current_date: 2026-09-12T23:24:04.130Z
---
...
For deployment patterns and end-to-end examples, see the [Dynamic Base URL guide](https://better-auth.com/docs/guides/dynamic-base-url).
...
| --- | --- | --- |
| `http://*.example.com` | `http://api.example.com`   `http://app.example.com`   `http://api.app.example.com` | `https://api.example.com`   `http://example.com` |
| `https://**.example.com` | `https://api.example.com`   `https://api.app.example.com` | `http://api.example.com` |
| `https://example.com` | `https://example.com`   `https://example.com/path` | `https://api.example.com`   `http://example.com` |
| `exp://192.168.*.*:*/**` | `exp://192.168.1.100:8081/path`   `exp://192.168.50.200:19000/auth/callback` | `exp://10.0.0.29:8081/path` |
...
Better Auth supports various database configurations including [PostgreSQL](https://better-auth.com/docs/adapters/postgresql), [MySQL](https://better-auth.com/docs/adapters/mysql), and [SQLite](https://better-auth.com/docs/adapters/sqlite).

Read more about [databases](https://better-auth.com/docs/concepts/database).
...
Read more about [secondary storage](https://better-auth.com/docs/concepts/database#secondary-storage).
...
- `customSyntheticUser`: Function to build a custom synthetic user for email enumeration protection. Use when plugins add fields to the user table. See [email enumeration protection](https://better-auth.com/docs/authentication/email-password#plugins-that-add-user-fields).
...
- `generateId`: Controls how record IDs are generated. Accepts a custom function, `false`, `"serial"`, or `"uuid"` (default: [random base62 string](https://github.com/better-auth/better-auth/blob/main/packages/core/src/utils/id.ts)). See the [Database documentation](https://better-auth.com/docs/concepts/database#id-generation) for more info.
- `defaultFindManyLimit`: The default maximum number of records returned by the `findMany` adapter method. (default: `100`)
- `joins`: Enable database joins for adapters that support them. When disabled (default), related data is fetched via separate queries. See the [Database documentation](https://better-auth.com/docs/concepts/database#joins) for more info. (default: `false`)
- `validateSchema`: Compare the schema with the tables Better Auth writes before the first request and report every difference with its fix. See the [Database documentation](https://better-auth.com/docs/concepts/database#schema-validation) for more info. (default: `true` outside production)
...
For more details and examples, see the [Hooks documentation](https://better-auth.com/docs/concepts/hooks).
